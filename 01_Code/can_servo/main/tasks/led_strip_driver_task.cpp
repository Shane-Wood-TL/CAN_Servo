...
void led_strip_driver_task(void *pv){
    ...
    for(;;){
        ...
        if(old_motor_status == OVERCURRENT and old_motor_status != OVERCURRENT){
            xSemaphoreGive(motor_status_mutex);
            led_strip.set_color(255,0,255, 0, 0);
            old_motor_status = OVERCURRENT;
        }else if (old_motor_status == OVERTEMP and old_motor_status != OVERTEMP){
            xSemaphoreGive(motor_status_mutex);
            led_strip.set_color(255,0,0, 0, 0);
            old_motor_status = OVERTEMP;
        }else if (old_motor_status == SLEEP and old_motor_status != SLEEP){
            xSemaphoreGive(motor_status_mutex);
            led_strip.set_color(255,0,0, 0, 0);
